    ...
    receive(message, socket) {
        ...
        if (message.type === "locate") {
            ...
            this.send(message.from,
                ...
                message.payload.node.address.type == "relay" ? false : true
            );
            ...
            this.send(message.from, new Message("relay", { node: this.node }));
            this.node.address.type = "relay";

            //this.tryUpnp();

        } else if (message.type == "relay") {

            this.pool.addRelay(message.from);
            this.send(message.from, new Message("relok", { node: this.node }));

        } else if (message.type == "relok") {
